...
public class Elfo extends Heroi {
    ...
    // elfo. Agora os danos são passados diretamente.
    private String tipoDeArco; // indica o arco utilizado (Beta, Alpha, Sigma)
    private int danoArcoBase; // dano base do arco, antes da força (agora reflete o
    // dano da arma equipada)
    private int cura; // atributo único --> faz o elfo se curar um pouco a cada ataque
    // (5% do dano do arco)
    ...
    // Método para atualizar o arco (e consequentemente a arma equipada) com base no
    // NÍVEL
    public void atualizarArco() {
        ...
        // 3. Atualiza os atributos do Elfo com base na arma que ele REALMENTE tem
        // equipada agora
    ...
    /*
    Agora implementados via interface (AcaoDeCombate)
    @Override
    public void atacar(Personagem alvo) { // ataque base (dano do arco + força)
    // Usa o dano da arma equipada (que foi atualizada por atualizarArco) mais a
    força do Elfo
    ...
    System.out.println("Elfo atacou com " + (this.arma != null ?
    this.arma.getNomeCompleto() : "punhos") + " causando " + danoTotal + " de dano e
    curou " + cura + " de vida!");
    }
    @Override
    ...
    danoEspecial += 15;
    System.out.println("A sorte favorece! Flecha Magica causa dano extra!");
    } else if (getSorte() < 0.25) { // Se a sorte for baixa, dano reduzido
    danoEspecial -= 5;
    System.out.println("O Elfo nao esta com sorte. Flecha Magica causa menos
    dano.");
    }
    alvo.receberDano(danoEspecial);
    // Usa o nome completo da arma para a mensagem
    System.out.println("Elfo usou Flecha Magica com " + (this.arma != null ?
    this.arma.getNomeCompleto() : "punhos") + " causando " + danoEspecial + " de dano
    especial!");
    }
    */
